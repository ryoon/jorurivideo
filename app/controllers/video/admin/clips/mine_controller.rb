# encoding: utf-8
class Video::Admin::Clips::MineController < Video::Admin::ClipsController
  ...
  def show
    init_action
    @item = Video::Clip.new.mine.find(params[:id])
    _show @item
  end
  ...
  def create
    ...
    _create @item, switch_redirect_url_options(:mine) do
  ...
  def update
    ...
    _update @item, switch_redirect_url_options(:mine)
  end
  ...
  def download
    item = Video::Clip.new
    #item.mine unless @admin_is
    item.and :id, params[:id]
    return error_auth unless @file = item.find(:first)
    return error_auth if !@file.sharable? && !@file.mine? && !@admin_is
    ...
    if params[:thumb]
      ...
      #redirect_to "/_common/test/jwplayer/video2.flv?start=#{params[:start]}"  #擬似ストリーミング
    end
  end
