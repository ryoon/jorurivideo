...
#    layout_id = (!change.layout_id.blank?) ? change.layout_id : change.old_layout_id
    ...
    if temp.parent_id
      t_parent_id = temp.parent_id
      t_level_no  = temp.entity_parent && !temp.entity_parent.level_no.blank? ? temp.entity_parent.level_no + 1 : change.level_no;
    else
      if p = Sys::Group.find(:first, :conditions => {:code => change.parent_code, :name => change.parent_name})
        t_parent_id = p.id
      end
      t_level_no = change.level_no
    end
  ...
  def _move_target_group(change)
    if og = Sys::Group.find(:first, :conditions => {:code => change.old_code, :name => change.old_name})
      if g = Sys::Group.find(:first, :conditions => {:code => change.code, :name => change.name})
        if g.parent && g.parent.code != change.parent_code
          # 対象
          temp_g = self.class.new({
            :group_change_id   => change.id,
            :group_id          => og.id,
  ...
  def _move_group(change, temp)
    if og = Sys::Group.find(:first, :conditions => {:code => change.old_code, :name => change.old_name})
      ...
      unless g = Sys::Group.find(:first, :conditions => {:code => change.code, :name => change.name})
        ...
        if temp.entity_parent
          update_part += ", parent_id = '#{temp.parent_id}'"
        else
          if p = Sys::Group.find(:first, :conditions => {:code => change.parent_code, :name => change.parent_name})
            update_part += ", parent_id = '#{p.id}'"
          end
        end
      ...
      if g
        if moved == false
          if p = Sys::Group.find(:first, :conditions => {:code => change.parent_code, :name => change.parent_name})
            sql = "UPDATE #{Sys::Group.table_name} SET parent_id = '#{p.id}' WHERE code = '#{change.old_code}'"
            Sys::Group.connection.execute(sql)
            g = Sys::Group.find(:first, :conditions => ["code = ? AND 0 = 0", change.code])
          end
        end
