...
class Video::Base::Status < PassiveRecord::Base
...
#  create :completed,      :name => '完了'
  ...
  def name
    ...
    when 'group';      return '所属公開'
    when 'public';     return '公開'
    when 'closed';     return '非公開'
    when 'completed';  return '完了'
    when 'private';    return 'マイ動画'
    when 'shared';     return '所属動画'
    when 'queue';      return '変換待ち'
    when 'start';      return '変換中'
    when 'done';       return '完了'
    when 'fail';       return '変換失敗'
    end
    nil
  end
